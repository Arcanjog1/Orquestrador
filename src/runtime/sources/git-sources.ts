...
export function gitExecutableNames(target: RuntimeTarget): string[] {
 ...
 */
export class MinGitReleaseSource implements RuntimeSource {
  readonly id = 'mingit-official-release';
  readonly label = 'MinGit (distribuição portátil oficial do Git for Windows)';
  readonly contract = 'DOCUMENTED' as const;
  readonly integrityStrategy: IntegrityStrategy = 'SHA256';

  constructor(
    private readonly releasesApi = 'https://api.github.com/repos/git-for-windows/git/releases/latest',
    private readonly fetchImpl: typeof fetch = fetch,
  ) {}

  async resolve(target: RuntimeTarget, _request: VersionRequest): Promise<ResolvedDownload | null> {
...
export function defaultGitSources(fetchImpl: typeof fetch = fetch): RuntimeSource[] {
  return [new MinGitReleaseSource(undefined, fetchImpl)];
}

interface MinGitAsset {
  url: string;
  version: string;
 ...
 */
function pickMinGitAsset(release: unknown, target: RuntimeTarget): MinGitAsset | null {
  ...
  let fallback: MinGitAsset | null = null;
  for (const raw of assets) {
    ...
    if (!/^MinGit-/i.test(name) || !name.toLowerCase().endsWith('.zip')) continue;
    if (!name.includes(bits)) continue;

    const entry: MinGitAsset = {
      url,
      version,
