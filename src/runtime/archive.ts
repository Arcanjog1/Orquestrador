...
import type { ArchiveKind } from './types.js';
...
export async function extractArchive(options: ExtractOptions): Promise<void> {
  if (options.kind === 'raw') return; // A bare executable needs no extraction.
  ...
  if (result.outcome !== 'completed' || result.exitCode !== 0) {
    throw new ExtractionError(
      options.archivePath,
      result.error ?? result.stderr.split(/\r?\n/)[0] ?? `tar exited ${result.exitCode}`,
    );
  }
}
 ...
 */
export function findExecutable(root: string, candidateNames: readonly string[]): string | null {
...
): { promoteDir: string; executableRelativePath: string } {
  return {
