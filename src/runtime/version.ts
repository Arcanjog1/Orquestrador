...
export interface ParsedVersion {
  ...
  minor: number;
  patch: number;
  /** A genuine prerelease tag, once any platform suffix has been removed. */
  prerelease: string | null;
  /** The platform suffix that was stripped, if any. */
  platformSuffix: string | null;
  raw: string;
}

const PLATFORM_SUFFIX =
  /-(?:win32|windows|darwin|macos|linux)-(?:x64|arm64|x86_64|aarch64|amd64)$/i;

export function parseVersion(raw: string): ParsedVersion | null {
  const trimmed = raw.trim().replace(/^v/i, '');

  let platformSuffix: string | null = null;
  let core = trimmed;
...
export function sameRelease(version: string, other: string): boolean {
  return compareVersions(version, other) === 0;
}

export function gte(version: string, floor: string): boolean {
  return compareVersions(version, floor) >= 0;
}

export function lte(version: string, ceiling: string): boolean {
