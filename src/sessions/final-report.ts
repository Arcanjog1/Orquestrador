...
import type { CommandResult, GitEvidence } from '../core/types.js';
...
export interface FinalReportInput {
  state: RunState;
  /** Evidence gathered at the end of the run, when it could be collected. */
  finalEvidence: GitEvidence | null;
  /** Warnings worth surfacing to the reader (dirty tree, skipped checks). */
...
export function buildFinalReport(input: FinalReportInput): string {
  ...
  lines.push(
    '## Status',
    '',
    `**${state.status}** - ${describeStatus(state.status)}`,
    '',
    `- Started: ${state.createdAt}`,
    `- Finished: ${state.finishedAt ?? '(not finished)'}`,
    `- Duration: ${formatDuration(state.createdAt, state.finishedAt ?? state.updatedAt)}`,
    `- Mode: ${state.mode === 'mock' ? 'mock agents' : 'real agent CLIs'}`,
    `- Project: ${state.projectPath}`,
  );
  if (state.terminationReason) lines.push(`- Reason: ${state.terminationReason}`);
  lines.push('');

  lines.push(
    '## Iterations',
    '',
    `${state.iteration} of a maximum ${state.maxIterations}.`,
    '',
  );
  ...
  const profiles = new Set(
    state.iterations.map((it) => it.worker?.profile).filter((p): p is string => Boolean(p)),
  );
  if (state.claudeProfile) profiles.add(state.claudeProfile);
  ...
  if (results.length === 0) {
    ...
    for (const r of results) {
      lines.push(
        `| \`${escapeCell(r.command)}\` | ${r.exitCode ?? '-'} | ${formatMs(r.durationMs)} | ${describeResult(r)} |`,
      );
    }
    lines.push('');
  }
  ...
  lines.push('## Evidence', '');
  lines.push(
    `Per-iteration evidence, prompts and raw agent output are under \`iterations/\` in this run's`,
    'directory. Each iteration holds `evidence.json`, `git-diff.patch`, `git-status.txt` and',
    '`tests.json`, all collected by the orchestrator rather than reported by an agent.',
    '',
  );
  ...
  if (state.baseline?.dirty) {
    warnings.push(
      'The working tree already had uncommitted changes before this run started. They were ' +
  ...
  if (!state.baseline) {
    ...
    lines.push(
      `- Commit: ${state.baseline.commit ?? '(no commits yet)'}`,
      `- Branch: ${state.baseline.branch ?? '(detached or unborn)'}`,
      `- Captured: ${state.baseline.capturedAt}`,
      `- Working tree at start: ${state.baseline.dirty ? 'dirty' : 'clean'}`,
      '',
    );
  }
...
function lastDoneGateResults(state: RunState): CommandResult[] {
  for (let i = state.iterations.length - 1; i >= 0; i -= 1) {
    const gate = state.iterations[i]?.doneRejection;
    if (gate) return gate.verification;
  }
  return [];
}
