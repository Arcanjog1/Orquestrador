...
export function assertReadOnlyGitArgs(args: readonly string[]): void {
  ...
  if (!subcommand) throw new UnsafeGitCommandError('Refusing to run git with no subcommand.');
  if (!READ_ONLY_GIT_SUBCOMMANDS.includes(subcommand)) {
    throw new UnsafeGitCommandError(
      `Refusing to run "git ${subcommand}": evidence collection is limited to read-only commands.`,
    );
  }
  if (subcommand === 'branch') {
    const mutating = args.some((a) =>
      ['-d', '-D', '--delete', '-m', '-M', '--move', '-c', '-C', '--copy', '-f', '--force'].includes(a),
    );
    if (mutating) throw new UnsafeGitCommandError('Refusing to run a mutating "git branch" command.');
  }
...
export interface CommandScreen {
  safe: boolean;
  /** Present when `safe === false`; shown to the user and to the orchestrator. */
  reason?: string;
}
 ...
 */
export function screenCommand(commandLine: string): CommandScreen {
  ...
  for (const op of SHELL_OPERATORS) {
    if (trimmed.includes(op)) {
      return {
        safe: false,
        reason:
          `Contains the shell operator "${op}". Verification commands are run without a shell, ` +
 ...
 */
function screenGitTokens(tokens: readonly string[]): string | null {
  ...
  const has = (...names: string[]): boolean => names.some((n) => flags.includes(n));
  ...
  switch (subcommand) {
    ...
    case 'switch':
      ...
      return null;
    case 'restore':
      if (flags.length === 1 && has('--staged')) return null;
      return 'git restore would overwrite working tree changes.';
    case 'branch':
      if (has('-D', '-d', '--delete')) return 'git branch --delete removes a branch.';
      return null;
    case 'push':
    ...
    case 'update-ref':
      if (has('-d')) return 'git update-ref -d deletes a ref.';
      return null;
    case 'worktree':
      if (positional[1]?.toLowerCase() === 'remove') return 'git worktree remove deletes a worktree.';
      return null;
    default:
      return null;
  }
...
export function parseCommandLine(line: string): string[] {
  ...
  if (quote) throw new CommandParseError(`Unbalanced ${quote} quote in command: ${line}`);
