...
import type { SqlRow } from '../../database/driver.js';

export interface PrincipalRecord extends SqlRow {
  id: string;
  display_name: string;
  tenant_id: string;
  status: string;
...
export interface RemoteRunRecord extends SqlRow {
  id: string;
  principal_id: string;
  tenant_id: string;
...
export class RunStore {
  ...
  createPrincipal(input: { displayName: string; tenantId?: string }): PrincipalRecord {
    const id = `pr_${randomUUID()}`;
    this.db.run(
      'INSERT INTO principals (id, display_name, tenant_id, status, created_at) VALUES (?,?,?,?,?)',
      [id, input.displayName, input.tenantId ?? id, 'active', new Date().toISOString()],
    );
    return this.db.get<PrincipalRecord>('SELECT * FROM principals WHERE id = ?', [id])!;
  }
  ...
  issueSession(input: {
    ...
    label?: string;
    ttlMs?: number;
  }): { id: string; token: string; expiresAt: string | null } {
    ...
    this.db.run(
      `INSERT INTO desktop_sessions (id, principal_id, token_hash, label, created_at, last_seen_at, expires_at, revoked_at)
       VALUES (?,?,?,?,?,NULL,?,NULL)`,
      [id, input.principalId, hashToken(token), input.label ?? null, new Date().toISOString(), expiresAt],
    );
   ...
   */
  authenticate(token: string): PrincipalRecord | null {
    if (!token) return null;
    const session = this.db.get<{ id: string; principal_id: string; expires_at: string | null; revoked_at: string | null }>(
      'SELECT id, principal_id, expires_at, revoked_at FROM desktop_sessions WHERE token_hash = ?',
      [hashToken(token)],
    );
    if (!session || session.revoked_at) return null;
    if (session.expires_at && session.expires_at <= new Date().toISOString()) return null;
    const principal = this.db.get<PrincipalRecord>(
      "SELECT * FROM principals WHERE id = ? AND status = 'active'",
  ...
  createRun(input: {
    ...
    objective: string;
    team?: unknown;
  }): { run: RemoteRunRecord; created: boolean } {
    return this.database.transaction(() => {
      if (input.idempotencyKey) {
        const existing = this.db.get<{ result_id: string }>(
          ...
          [input.principal.id, 'run.create', input.idempotencyKey],
        );
        if (existing) {
      ...
      if (input.idempotencyKey) {
        this.db.run(
          'INSERT INTO idempotency_keys (key, principal_id, scope, result_id, created_at) VALUES (?,?,?,?,?)',
          [input.idempotencyKey, input.principal.id, 'run.create', id, timestamp],
        );
      }
      const run = this.db.get<RemoteRunRecord>('SELECT * FROM remote_runs WHERE id = ?', [id])!;
   ...
   */
  findRun(id: string, principal: PrincipalRecord): RemoteRunRecord | null {
    return (
      this.db.get<RemoteRunRecord>('SELECT * FROM remote_runs WHERE id = ? AND principal_id = ?', [
        id,
        principal.id,
      ]) ?? null
    );
  ...
  listRuns(principal: PrincipalRecord, limit = 50): RemoteRunRecord[] {
    return this.db.all<RemoteRunRecord>(
      'SELECT * FROM remote_runs WHERE principal_id = ? ORDER BY created_at DESC LIMIT ?',
      [principal.id, limit],
    );
  }

  /** Used by the coordinator itself, which acts for every principal. */
  requireRunUnscoped(id: string): RemoteRunRecord {
    const run = this.db.get<RemoteRunRecord>('SELECT * FROM remote_runs WHERE id = ?', [id]);
    if (!run) throw new Error(`no remote run ${id}`);
    return run;
  }

  setStatus(id: string, status: string, failureReason?: string | null): void {
    const timestamp = new Date().toISOString();
    this.db.run(
      `UPDATE remote_runs
        ...
        WHERE id = ?`,
      [status, failureReason ?? null, timestamp, status, timestamp, id],
    );
    this.append(id, 'run.status', { status, failureReason: failureReason ?? null });
  }

  setCloudWorkspace(id: string, cloudWorkspaceId: string | null): void {
   ...
   */
  append(runId: string, kind: string, payload: unknown): number {
    return this.database.transaction(() => {
      const row = this.db.get<{ next: number | null }>(
        'SELECT MAX(seq) AS next FROM remote_run_events WHERE run_id = ?',
        [runId],
      );
      const seq = (row?.next ?? 0) + 1;
      this.db.run(
        'INSERT INTO remote_run_events (run_id, seq, kind, payload, created_at) VALUES (?,?,?,?,?)',
        [runId, seq, kind, JSON.stringify(payload ?? null), new Date().toISOString()],
      );
      return seq;
    });
  }
  ...
  events(runId: string, afterSeq = 0, limit = 500): RunEvent[] {
    return this.db
      .all<{ seq: number; kind: string; payload: string; created_at: string }>(
        'SELECT seq, kind, payload, created_at FROM remote_run_events WHERE run_id = ? AND seq > ? ORDER BY seq ASC LIMIT ?',
  ...
  acquireLease(runId: string, owner: string, ttlMs: number): boolean {
    return this.database.transaction(() => {
      ...
      const held = this.db.get<{ owner: string; expires_at: string }>(
        'SELECT owner, expires_at FROM run_leases WHERE run_id = ?',
        [runId],
      );
      if (held && held.expires_at > nowIso && held.owner !== owner) return false;
      ...
      if (held) {
        ...
        this.db.run(
          'INSERT INTO run_leases (run_id, owner, acquired_at, expires_at) VALUES (?,?,?,?)',
          [runId, owner, nowIso, expiresAt],
        );
      }
      return true;
    });
  ...
  renewLease(runId: string, owner: string, ttlMs: number): boolean {
    const result = this.db.run(
      'UPDATE run_leases SET expires_at = ? WHERE run_id = ? AND owner = ?',
      [new Date(Date.now() + ttlMs).toISOString(), runId, owner],
    );
   ...
   */
  listAbandoned(): RemoteRunRecord[] {
    return this.db.all<RemoteRunRecord>(
      `SELECT r.* FROM remote_runs r
