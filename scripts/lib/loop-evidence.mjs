...
export function printLoopEvidence(evidence, log = console.log) {
  ...
  for (const t of orchestratorTurns) {
    log(`  codex turn it${t.iteration} ${t.invocationId}: ${t.outcome} exit=${t.exitCode ?? 'null'} ${t.durationMs ?? '?'}ms`);
  }
  for (const t of workerTurns) {
    log(`  claude turn it${t.iteration} ${t.invocationId}: ${t.outcome} exit=${t.exitCode ?? 'null'} ${t.durationMs ?? '?'}ms`);
