...
test('model display names are centralized and canonical IDs never change',()=>{assert.equal(modelDisplayName('anthropic','opus'),'Opus');assert.equal(modelDisplayName('anthropic','claude-sonnet-4-6','Sonnet 4.6'),'Sonnet 4.6');assert.equal(modelDisplayName('openai','gpt-5.3-codex-spark'),'GPT-5.3-Codex-Spark');assert.equal(reasoningName('xhigh'),'Muito alto');const r=decorateAgentModels(rows(['opus','sonnet','haiku']),account,config);assert.deepEqual(r.map(m=>m.id),['opus','sonnet','haiku']);assert.deepEqual(r.map(m=>m.displayName),['Opus','Sonnet','Haiku']);});
