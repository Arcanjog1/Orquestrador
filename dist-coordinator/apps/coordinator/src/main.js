...
import { DEFAULT_LIMITS } from '../../../src/cloud/provisioner.js';
function required(name) {
    const value = process.env[name];
    if (!value) {
        throw new Error(`${name} não está definido. O coordenador não sobe sem ele: aceitar execuções que não pode executar é pior do que recusar iniciar.`);
    }
    return value;
}
...
async function issueToken(label) {
    ...
    try {
        ...
        const existing = database.driver.get('SELECT * FROM principals WHERE display_name = ? AND status = \'active\'', [principalName]);
...
export async function main() {
    ...
    if (command === 'issue-token') {
        ...
        if (!label)
            throw new Error('uso: issue-token "<nome do dispositivo>"');
    ...
    if (recovered > 0)
        console.log(`[coordinator] recuperadas ${recovered} execuções`);
