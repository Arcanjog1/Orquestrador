...
import { DECISION_JSON_SCHEMA, ProcessManager, newId, } from '../../desktop/src/main/core.js';
import { EventBus } from '../../desktop/src/main/events.js';
import { OrchestrationService, } from '../../desktop/src/main/services/orchestration-service.js';
...
import { DEFAULT_LIMITS, ProvisioningError, } from '../../../src/cloud/provisioner.js';
...
export class Coordinator {
    ...
    async submit(input) {
        ...
        if (result.created)
            this.track(this.drive(result.run.id));
        return result;
    }
    /**
    ...
    async shutdown() {
        ...
        for (const controller of this.aborts.values())
            controller.abort();
        for (const driving of this.driving.values())
            driving.orchestration.cancel(driving.localRunId);
    ...
    async drive(runId) {
        if (this.stopped)
            return;
        if (!this.store.acquireLease(runId, this.owner, this.options.leaseTtlMs ?? LEASE_TTL_MS))
            return;
        ...
        try {
            ...
            if (this.stopped)
                return;
            const reason = error instanceof ProvisioningError
                ? error.userMessage
     ...
     */
    async signIn(workspace) {
        const key = this.options.credentials.openaiApiKey;
        if (!key) {
            throw new ProvisioningError('REPOSITORY_UNAUTHORIZED', 'O ambiente remoto não tem uma credencial autorizada para o Codex.', 'configure OPENAI_API_KEY no coordenador');
        }
        ...
        if (result.exitCode !== 0) {
            throw new ProvisioningError('REPOSITORY_UNAUTHORIZED', 'O Codex não aceitou a credencial no ambiente remoto.', redact(firstLine(result.stderr) ?? `saída ${result.exitCode}`));
        }
    }
    /**
     ...
     */
    async runLoop(run, workspace, cloudWorkspaceId) {
        ...
        const orchestration = new OrchestrationService(this.database, this.processManager, events, async () => this.buildRunners(environment), {
        ...
        const view = await orchestration.waitFor(started.id, (this.options.agentTimeoutMs ?? 15 * 60_000) * ((this.options.maxIterations ?? 8) + 2));
        return { status: view.status, reason: view.summary ?? null };
    }
    /** The agent pair, built against the workspace rather than this machine. */
    buildRunners(environment) {
     ...
     */
    async ensureLocalWorkspaceRow(run) {
    ...
    teamOf(run) {
        try {
            const parsed = JSON.parse(run.team || '{}');
