...
import { describeProbe, modelAliases, optionValues, readCapabilities, } from './cli-capabilities.js';
export class ClaudeCodeAdapter {
     ...
     */
    async describeCapabilities(cwd = process.cwd()) {
        const executable = await this.options.resolveExecutable();
     ...
     */
    async readHelp(executable, cwd) {
        if (this.capabilities)
            return this.capabilities;
        const capabilities = await readCapabilities(this.options.processManager, executable, cwd, ['--help'], this.options.buildEnvironment());
     ...
     */
    async buildArgs(executable, cwd, routing) {
        const capabilities = await this.readHelp(executable, cwd);
        if (!capabilities.flags.has('--print')) {
            throw new ClaudeCapabilityError('CAPABILITY_UNSUPPORTED', 'Esta versão do Claude Code não oferece um modo não interativo compatível.', `o executável respondeu, mas sua ajuda não declara --print (${describeProbe(capabilities.probe)})`, executable);
        }
        const args = ['--print'];
...
export class ClaudeCapabilityError extends Error {
    ...
    executable;
    userMessage;
    constructor(reason, message, diagnosis = null, executable = null) {
