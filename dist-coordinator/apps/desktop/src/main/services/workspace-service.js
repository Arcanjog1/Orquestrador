...
export class WorkspaceService {
    ...
    processManager;
    agents;
    constructor(database, runtimes, processManager, agents = new AgentService(database)) {
    ...
    rename(workspaceId, name) {
        ...
        if (trimmed.length === 0)
            throw new WorkspaceError('Escolha um nome para o projeto.');
    ...
    async branches(workspaceId) {
        ...
        if (!(await collector.isGitRepository()))
            return none;
     ...
     */
    async checkout(workspaceId, branch, allowDirty) {
        const workspace = this.database.workspaces.require(workspaceId);
        if (this.isBusy(workspaceId)) {
        ...
        if (!known.isRepository)
            throw new WorkspaceError('Esta pasta não é um repositório git.');
    ...
    async listWithBranches() {
        ...
        return Promise.all(records.map(async (record) => ({
            ...
            branch: record.environment === 'cloud'
                ? record.branch
                : await this.currentBranch(record.local_path),
        })));
    }
    async currentBranch(localPath) {
        let git;
        try {
            git = await this.runtimes.executablePath('git');
        }
    ...
    create(input) {
        ...
        if (existing)
            throw new WorkspaceError('Esta pasta já está adicionada como projeto.');
    ...
    createCloud(input) {
        ...
        if (!branch)
            throw new WorkspaceError('Escolha a branch em que o trabalho começa.');
    ...
    async clone(input) {
        ...
        if (result.outcome !== 'completed' || result.exitCode !== 0) {
            throw new WorkspaceError(`Não foi possível clonar o repositório.${firstLine(result.stderr)}`);
        }
        return this.create({
            name: input.name,
    ...
    setAgents(workspaceId, orchestratorAgentId, workerAgentId) {
        ...
        return this.toView(this.database.workspaces.setAgents(workspaceId, orchestratorAgentId, workerAgentId));
    }
    /**
    ...
    setTeam(workspaceId, orchestrator, worker) {
        ...
        const record = this.database.workspaces.setTeam(workspaceId, {
     ...
     */
    async runGit(workspaceId, args, summaryOk, summaryFail, options = {}) {
        const workspace = this.database.workspaces.require(workspaceId);
        if (this.isBusy(workspaceId)) {
    ...
    createBranch(workspaceId, name) {
        return this.runGit(workspaceId, ['switch', '--create', name], `Branch "${name}" criada e ativa.`, 'Não foi possível criar a branch.');
    }
    /**
    ...
    async commit(workspaceId, message) {
        ...
        if (!configured && !login) {
            ...
            return {
                ok: false,
                summary: 'O git não sabe quem você é. Conecte o GitHub em Contas e integrações, ou configure user.name e user.email no git.',
                output: '',
        ...
        const identity = login
            ...
            : {};
        return this.runGit(workspaceId, ['commit', '--message', message], 'Commit criado.', 'O commit não foi criado.', { extraEnv: configured ? {} : identity });
    }
    async hasIdentity(workspaceId) {
        const workspace = this.database.workspaces.require(workspaceId);
        try {
            const git = await this.runtimes.executablePath('git');
            const collector = new GitEvidenceCollector(workspace.local_path, this.processManager, git);
    ...
    push(workspaceId) {
        return this.runGit(workspaceId, ['push', '--set-upstream', 'origin', 'HEAD'], 'Push concluído.', 'O push falhou.');
    }
    async pullRequestStatus(workspaceId) {
        const workspace = this.database.workspaces.require(workspaceId);
    ...
    async createPullRequest(input) {
        ...
        if (!this.github)
            throw new WorkspaceError('O GitHub não está disponível.');
        const remoteUrl = await this.remoteUrl(workspace);
        if (!remoteUrl)
            throw new WorkspaceError('Este projeto não tem um remoto configurado.');
        const head = await this.currentBranch(workspace.local_path);
        if (!head)
            throw new WorkspaceError('O projeto não está em uma branch.');
        const base = input.base ?? workspace.default_branch ?? (await this.defaultBranchOf(workspace)) ?? 'main';
        if (base === head)
            throw new WorkspaceError(`A branch atual já é "${base}"; crie uma branch para o pull request.`);
    ...
    async defaultBranchOf(workspace) {
        try {
            const git = await this.runtimes.executablePath('git');
            const collector = new GitEvidenceCollector(workspace.local_path, this.processManager, git);
    ...
    accountForRole(role, accountId) {
        const account = this.database.accounts.find(accountId);
        if (!account)
            throw new WorkspaceError('Essa conta não existe mais.');
        ...
        if (account.provider_id !== expected) {
            ...
            throw new WorkspaceError(`A conta "${account.display_name}" não pode ${job}: esta função precisa de uma conta ${provider}.`);
        }
        return account;
    }
    toView(record) {
        return {
            ...
            team: {
                orchestrator: this.memberView('ORCHESTRATOR', record.orchestrator_agent_id, record.orchestrator_model, record.orchestrator_reasoning, orchestratorSelectionOf(record)),
                worker: this.memberView('CODING_WORKER', record.worker_agent_id, record.worker_model, record.worker_reasoning, isWorkerSelection(record.worker_selection) ? record.worker_selection : 'auto'),
            },
            createdAt: record.created_at,
            updatedAt: record.updated_at ?? record.created_at,
        };
    }
    memberView(role, agentId, model, reasoning, selection) {
...
function readTextIfSmall(path) {
    try {
        ...
        if (!stats.isFile() || stats.size > 64_000)
            return null;
...
export function orchestratorSelectionOf(record) {
    if (record.orchestrator_selection === 'manual')
        return 'manual';
    if (record.orchestrator_selection === null && (record.orchestrator_model || record.orchestrator_reasoning))
        return 'manual';
    return 'auto';
}
function reasoningOrNull(value) {
    return REASONING_LEVELS.includes(value ?? '')
        ? value
        : null;
}
function assertUsableDirectory(path) {
    let stats;
    try {
        stats = statSync(path);
    }
    ...
    if (!stats.isDirectory())
        throw new WorkspaceError('O caminho escolhido não é uma pasta.');
    try {
        readdirSync(path);
    }
    catch {
...
export function safeFolderName(name) {
    const cleaned = basename(name.trim())
        ...
        .trim();
    if (cleaned.length === 0)
        throw new WorkspaceError('Escolha um nome de projeto válido.');
    return cleaned;
}
