 ...
 */
import { ClaudeAccountManager, CodexAccountManager, Database, ProcessManager, RuntimeManager, appPaths, isUsable, } from '../core.js';
...
export class AppServices {
    ...
    constructor(options = {}) {
        ...
        this.accounts = new AccountService(this.database, { anthropic: this.accountManager, openai: this.codexAccountManager }, this.events, options.openUrl ?? (() => { }));
        this.agents = new AgentService(this.database);
        this.workspaces = new WorkspaceService(this.database, this.runtimes, this.processManager, this.agents);
        this.verifications = new VerificationService(this.database);
        this.orchestration = new OrchestrationService(this.database, this.processManager, this.events, options.createRunners ?? ((workspace) => this.buildRunners(workspace)), this.orchestrationOptions(options.orchestration ?? {}), 
        ...
        this.github = new GitHubService(this.database, options.secrets ?? NO_SECRET_STORE, this.events, options.openUrl ?? (() => { }), options.github ?? {});
     ...
     */
    orchestrationOptions(explicit) {
        const setting = (key, min, max) => {
     ...
     * silent minutes.
     */
    async checkAgentsReady(workspace) {
        for (const [agentId, what, provider] of [
            ...
            if (!agentId)
                return `Escolha a conta ${what} este projeto em Equipe.`;
            const agent = this.database.agents.find(agentId);
            if (!agent)
                return `O agente ${what} este projeto não existe mais. Escolha a conta em Equipe.`;
            ...
            const manager = managers[record.provider_id];
            if (!manager)
                continue;
