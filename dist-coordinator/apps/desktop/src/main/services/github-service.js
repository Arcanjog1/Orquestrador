 ...
 */
import { GitHubClient, GitHubError, gitAuthEnvironment, isGitHubHttpsRemote, parseGitHubRemote, } from '../core.js';
...
export class GitHubService {
    ...
    client;
    connecting = null;
    constructor(database, secrets, events, openUrl, clientOptions = {}) {
    ...
    configure(clientId) {
        ...
        if (/^\d+$/.test(trimmed)) {
            throw new GitHubServiceError('Isso parece o App ID (um número). O Client ID é o texto que começa com "Iv1." ou "Iv23li", na mesma página do app.');
        }
    ...
    async connect() {
        ...
        if (!this.secrets.available) {
            throw new GitHubServiceError('Este sistema não oferece armazenamento protegido para o login do GitHub, então ele não será guardado.');
        }
        if (this.connecting)
            return this.status();
        const controller = new AbortController();
        this.connecting = controller;
        const report = (stage, label, code, detail) => this.events.emit('account:progress', {
    ...
    async branches(repository) {
        ...
        if (!owner || !name)
            throw new GitHubServiceError('Escolha um repositório no formato dono/nome.');
    ...
    async createPullRequest(input) {
        ...
        if (!remote)
            throw new GitHubServiceError('O remoto deste projeto não é um repositório do GitHub.');
    ...
    storeToken(token) {
        ...
        if (token.refreshToken)
            settings.set(KEYS.refreshToken, this.secrets.encrypt(token.refreshToken));
        else
            settings.remove(KEYS.refreshToken);
        if (token.expiresAt)
            settings.set(KEYS.expiresAt, String(token.expiresAt));
        else
            settings.remove(KEYS.expiresAt);
    ...
    async accessToken() {
        ...
        if (!stored)
            throw new GitHubServiceError('Conecte o GitHub em Contas e integrações.', 'GITHUB_NOT_CONNECTED');
...
function detailOf(error) {
    if (error instanceof GitHubError)
        return error.detail ?? (error.status ? `HTTP ${error.status}` : null);
