...
export class CloudAccountService {
    ...
    secrets;
    makeClient;
    constructor(database, secrets, makeClient = (endpoint, token) => new CloudClient({ endpoint, token })) {
    ...
    async connect(input) {
        if (!this.secrets.available) {
            throw new CloudAccountError('Este sistema não oferece armazenamento protegido para credenciais, então o acesso à nuvem não pode ser guardado aqui.');
        }
...
//# sourceMappingURL=cloud-account-service.js.map