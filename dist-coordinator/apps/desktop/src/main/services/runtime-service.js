...
export class RuntimeService {
    ...
    database;
    running = new Map();
    constructor(runtimeManager, events, database) {
    ...
    async diagnose() {
        ...
        const runtimes = report.runtimes.map((status) => ({
    ...
    async executablePath(runtimeId) {
 ...
 */
function reasoningLevelsFor(runtimeId, version) {
    if (runtimeId !== 'codex' || !version)
        return null;
...
function describe(healthy, origin, problem) {
    if (healthy)
        return origin === 'managed' ? 'Pronto (gerenciado pelo aplicativo)' : 'Pronto';
