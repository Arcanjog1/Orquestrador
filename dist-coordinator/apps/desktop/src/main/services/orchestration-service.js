 ...
 */
import { AcceptanceCriteriaLedger, GitEvidenceCollector, Verifier, commandPassed, evaluateDone, formatDoneRejection, isMechanicalFailure, modelUnavailableIn, newId, parseDecision, redact, localEnvironment, routeWorkerModel, } from '../core.js';
...
export class OrchestrationService {
    ...
    constructor(database, processManager, events, createRunners, options = {}, checkReadiness = async () => null) {
        this.database = database;
        this.processManager = processManager;
        this.events = events;
        this.createRunners = createRunners;
     ...
     */
    async resolveEnvironment(workspace, signal) {
        if (this.options.environments)
            return this.options.environments(workspace, signal);
    ...
    hasActiveRunInWorkspace(workspaceId) {
        for (const runId of this.active.keys()) {
            if (this.database.runs.find(runId)?.workspace_id === workspaceId)
                return true;
        }
        return false;
    }
    ...
    cancel(runId) {
        ...
        if (environment && environment.kind !== 'local')
            void environment.processes.cancelAll();
        return true;
    }
    /**
    ...
    async waitFor(runId, timeoutMs = 120_000) {
    ...
    detail(runId) {
        const run = this.database.runs.require(runId);
        return toRunDetailView(run, this.database.runs.steps(runId), this.database.runs.invocations(runId), this.database.runs.verifications(runId));
    }
    // -- the loop ------------------------------------------------------------
    async execute(runId, workspace, objective, controller) {
        ...
        const verifier = new Verifier({
            ...
            ...(environment.kind === 'remote' ? { resolveCommand: async (command) => command } : {}),
        });
        ...
        const resolvedCommands = new Set();
        let feedback = null;
        ...
        for (let iteration = 1; iteration <= maxIterations; iteration += 1) {
            if (signal.aborted)
                return this.finishCancelled(runId, sessionId);
            ...
            previousTree = tree;
            this.step(runId, iteration, 'evidence', evidence.changedSinceBaseline ? 'changed' : 'unchanged', `${evidence.changedFiles.length} arquivo(s)`);
            if (evidence.changedSinceBaseline) {
                this.say(sessionId, runId, 'system', `Alterações coletadas: ${describeFiles(evidence)}`);
            }
            ...
            const allPassed = verification.length > 0 && verification.every(commandPassed) && unknownIds.length === 0;
            for (const criterion of decision.acceptanceCriteria) {
                ledger.markByText(criterion, allPassed ? 'satisfied' : 'failed', iteration, verificationNote(verification));
            }
     ...
     */
    conversationBefore(sessionId, runId) {
    ...
    async askForDecision(input) {
        ...
        for (let attempt = 1; attempt <= 2; attempt += 1) {
            ...
            if (result.applied)
                input.onApplied?.(result.applied);
            if (signal.aborted)
            ...
            if (result.outcome !== 'completed') {
                const problem = result.outcome === 'timeout'
        ...
        const failure = `O Codex respondeu duas vezes, mas não com uma decisão válida. ` +
    ...
    orchestratorAccountId(workspace) {
        if (!workspace.orchestrator_agent_id)
            return null;
        return this.database.agents.find(workspace.orchestrator_agent_id)?.account_id ?? null;
    }
    buildOrchestratorPrompt(input) {
        ...
        if (input.history) {
            lines.push('', 'CONVERSATION BEFORE THIS OBJECTIVE (for context; the OBJECTIVE above is what to do now):', input.history);
        }
        if (input.feedback) {
    ...
    buildFeedback(evidence, verification, unknownIds, record) {
        const lines = [];
        ...
        if (worker) {
            ...
            lines.push('WORKER OF THIS ITERATION:', `  requested: ${routing ? `${routing.requestedCapability ?? 'default'}/${routing.requestedReasoning ?? 'default'}` : '(not routed)'}`, `  ran as: model ${routing?.resolvedModel ?? '(CLI default)'}, reasoning ${routing?.resolvedReasoning ?? '(CLI default)'}` +
                (routing ? ` [${routing.selectionMode}${routing.fallbackUsed ? ', fallback' : ''}]` : ''), `  outcome: ${worker.outcome}${worker.exitCode !== null ? ` (exit ${worker.exitCode})` : ''}` +
                (worker.mechanical ? ' - a mechanical failure (binary, login, quota, network), not the model' : '') +
                (worker.modelUnavailable ? ' - the CLI refused the model' : ''), `  progressed: ${worker.progressed ? 'yes' : 'no'}`, '');
        }
        ...
        if (evidence.diffStat.trim())
            lines.push('  diffstat:', indent(evidence.diffStat));
        if (unknownIds.length > 0) {
            lines.push('', 'REFUSED: these verification ids are not registered for this workspace and were', `not executed: ${unknownIds.join(', ')}`);
        }
        if (verification.length > 0) {
            ...
            for (const result of verification) {
                ...
                if (verdict !== 'PASS' && output)
                    lines.push(indent(output.slice(0, 2000)));
            }
        }
        return lines.join('\n');
    }
    ...
    async delegate(input) {
        ...
        for (let attempt = 0; attempt <= MODEL_RETRIES; attempt += 1) {
            ...
            const planned = routed
                ...
                : null;
            this.say(sessionId, runId, 'worker', attempt === 0 ? `Executando: ${task}` : `Tentando outro modelo: ${task}`, planned ? { routing: planned } : undefined);
            const startedAt = new Date().toISOString();
            ...
            const recorded = planned
                ...
                : null;
            const modelUnavailable = routed?.resolvedModel !== null && routed?.resolvedModel !== undefined && modelUnavailableIn(result);
            ...
            this.say(sessionId, runId, 'system', `O modelo ${routed.resolvedModel} não está disponível nesta conta; tentando ${routed.alternatives[0]}.`);
        }
        return last;
    }
    finishCancelled(runId, sessionId) {
    ...
    step(runId, iteration, phase, status, summary, detail) {
    ...
    say(sessionId, runId, author, body, payload) {
    ...
    progress(runId, sessionId, stage, label, status) {
...
function treeKey(statusShort, diff) {
    return `${statusShort.trim()}\n${diff.length}:${diff.slice(0, 4000)}`;
}
function verificationNote(results) {
    if (results.length === 0)
        return 'Nenhuma verificação foi executada nesta iteração.';
...
function describeFiles(evidence) {
