...
export class CloudService {
    ...
    async cancel(localRunId) {
        ...
        const client = this.options.clientFor(workspace.cloud_endpoint);
        if (!client)
            throw new CloudServiceError('Sem conexão com a nuvem para cancelar.');
        const cancelled = await client.cancel(run.remote_run_id);
        if (cancelled)
            await this.sync(localRunId).catch(() => { });
        return cancelled;
    }
    // -- applying one event ---------------------------------------------------
    /**
     ...
     */
    apply(localRunId, sessionId, event) {
        ...
        if (event.kind === 'orchestration.run:progress') {
            ...
            if (sessionId && payload.message && typeof payload.message === 'object') {
                this.messageOnce(sessionId, localRunId, event.seq, payload.message);
            }
            return;
        }
    ...
    applyStatus(localRunId, status, failureReason) {
        ...
        if (current.status === mapped)
            return;
        this.database.runs.setStatus(localRunId, mapped, typeof failureReason === 'string' ? failureReason : null);
    }
    /**
    ...
    stepOnce(localRunId, seq, phase, summary) {
     ...
     */
    messageOnce(sessionId, runId, seq, message) {
    ...
    step(runId, iteration, phase, status, summary) {
...
function remoteSeqOf(payload) {
    if (!payload)
        return null;
    try {
        const parsed = JSON.parse(payload);
        const seq = parsed?.remoteSeq;
