...
export class ProjectService {
    ...
    setWorkspace(projectId, workspaceId) {
        this.require(projectId);
    ...
    require(projectId) {
        ...
        if (!record)
            throw new ProjectError('Este projeto não existe mais.');
        return record;
    }
    workspaceOrNull(workspaceId) {
        if (!workspaceId)
            return null;
...
function cleanName(name) {
    ...
    if (trimmed.length === 0)
        throw new ProjectError('Dê um nome ao projeto.');
    if (trimmed.length > 120)
        throw new ProjectError('O nome do projeto é longo demais (máximo 120 caracteres).');
    return trimmed;
}
