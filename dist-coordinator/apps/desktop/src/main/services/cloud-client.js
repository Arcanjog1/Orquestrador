...
export class CloudClient {
    ...
    async run(remoteRunId) {
        const body = await this.json('GET', `/v1/runs/${encodeURIComponent(remoteRunId)}`);
        return body.run;
    }
    async runs() {
        return (await this.json('GET', '/v1/runs')).runs;
    }
    /**
     ...
     */
    async events(remoteRunId, afterSeq, limit = 500) {
        return this.json('GET', `/v1/runs/${encodeURIComponent(remoteRunId)}/events?after=${afterSeq}&limit=${limit}`);
    }
    async cancel(remoteRunId) {
        const body = await this.json('POST', `/v1/runs/${encodeURIComponent(remoteRunId)}/cancel`);
        return body.cancelled;
    }
    async json(method, path, body, extraHeaders = {}) {
    ...
    async raw(method, path, body, extraHeaders = {}) {
        ...
        try {
            ...
            throw new CloudError('OFFLINE', 'Sem conexão com a nuvem. O trabalho continua lá; esta janela é que não conseguiu falar com ela.', error.message);
        }
        finally {
            clearTimeout(timer);
        }
