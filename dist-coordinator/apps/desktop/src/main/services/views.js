...
export function toSessionView(record, extra) {
    return {
        id: record.id,
...
export function toMessageView(record) {
    return {
        id: record.id,
...
function routingOfPayload(payload) {
    if (!payload)
        return null;
    try {
...
export function toRunView(record, steps) {
    return {
        id: record.id,
        sessionId: record.session_id ?? '',
        workspaceId: record.workspace_id,
...
export function toRunDetailView(record, steps, invocations, verifications) {
    const str = (v) => (typeof v === 'string' ? v : null);
    const num = (v) => (typeof v === 'number' ? v : null);
    const flag = (v) => v === 1 || v === true ? true : v === 0 || v === false ? false : null;
    return {
