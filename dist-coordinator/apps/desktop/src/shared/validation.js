 ...
 */
import { REASONING_LEVELS, WORKER_SELECTIONS } from './ipc-contract.js';
...
export const noArgs = (value, path) => {
    if (value !== undefined && value !== null)
        fail(path, 'takes no argument');
};
export function str(rules = {}) {
    ...
    return (value, path) => {
        ...
        if (value.includes('\0'))
            fail(path, 'must not contain a NUL byte');
        if (value.length < min)
            fail(path, `must be at least ${min} character(s)`);
        if (value.length > max)
            fail(path, `must be at most ${max} character(s)`);
 ...
 */
export function absolutePath(max = 4096) {
    return (value, path) => {
...
export const repositoryUrl = (value, path) => {
    const s = str({ min: 1, max: 2048 })(value, path);
    if (/\s/.test(s))
        fail(path, 'must not contain whitespace');
    if (s.startsWith('-'))
        fail(path, 'must not start with "-"');
    const ok = /^https:\/\/[^\s]+$/.test(s) ||
        /^git@[^\s:]+:[^\s]+$/.test(s) ||
        /^ssh:\/\/[^\s]+$/.test(s) ||
        /^file:\/\/\/[^\s]+$/.test(s);
    if (!ok)
        fail(path, 'must be an https, ssh or file repository URL');
    return s;
};
 ...
 */
export const verificationCommand = (value, path) => {
...
export function obj(shape, rules = {}) {
    ...
    return (value, path) => {
        ...
        for (const key of Object.keys(source)) {
            ...
            if (!(key in shape))
                fail(`${path}.${key}`, 'is not a known property');
        }
        ...
        for (const [key, validate] of Object.entries(shape)) {
            ...
            const takesNull = validate.nullable === true;
...
function nullable(inner) {
    const validate = (value, path) => value === null ? null : inner(value, path);
    validate.nullable = true;
    return validate;
}
export const runtimeId = oneOf(['codex', 'claude-code', 'git']);
/**
 ...
 */
export const repositoryFullName = (value, path) => {
 ...
 */
export const cloudEndpoint = (value, path) => {
    ...
    if (url.username || url.password)
        fail(path, 'must not carry a credential in the URL');
    return text;
};
/**
...
export const sessionTitle = (value, path) => {
 ...
 */
export const branchName = (value, path) => {
...
export const teamMember = obj({ accountId: id, model: modelName, reasoning: oneOf(REASONING_LEVELS), selection: oneOf(WORKER_SELECTIONS) }, { optional: ['model', 'reasoning', 'selection'] });
/**
 ...
 */
export const externalUrl = (value, path) => {
...
export const REQUEST_VALIDATORS = {
    ...
    'accounts.create': obj({
        ...
        provider: oneOf(['anthropic', 'openai']),
    }),
    ...
    'workspace.checkout': obj({ workspaceId: id, branch: branchName, allowDirty: bool }, { optional: ['allowDirty'] }),
    ...
    'chat.listSessions': obj({ workspaceId: id, includeArchived: bool, query: str({ min: 0, max: 200 }) }, { optional: ['includeArchived', 'query'] }),
    'chat.listAllSessions': obj({ includeArchived: bool, query: str({ min: 0, max: 200 }) }, { optional: ['includeArchived', 'query'] }),
    'chat.moveSession': obj({ sessionId: id, projectId: nullable(id) }),
    'chat.createSession': obj({ workspaceId: id, title: str({ min: 1, max: 200 }), projectId: nullable(id) }, { optional: ['projectId'] }),
    'project.list': noArgs,
    'project.create': obj({ name: str({ min: 1, max: 200 }), workspaceId: nullable(id) }, { optional: ['workspaceId'] }),
