 ...
 */
export function safeExcerpt(raw, max = 240) {
    const scrubbed = raw
...
export class GitHubClient {
        ...
        while (Date.now() < deadline) {
            ...
            switch (body.error) {
                ...
                case 'incorrect_client_credentials':
                    throw new GitHubError('O GitHub não aceitou o Client ID durante o login. Confira o Client ID do GitHub App.', 401, 'config', detail);
                ...
                case 'device_flow_disabled':
                    throw new GitHubError('O fluxo de dispositivo está desligado neste GitHub App. Ative "Enable Device Flow" na página do app.', 400, 'config', detail);
                default:
                    throw new GitHubError(`O GitHub recusou o login: ${String(body.error_description ?? body.error ?? `HTTP ${answer.status}`)}`, answer.status || 400, 'auth', detail);
            }
        }
    ...
    // -- REST ------------------------------------------------------------------
    async user(token) {
        const body = (await this.api(token, '/user'));
        return {
     ...
     */
    async repositories(token, maxPages = REPOS_MAX_PAGES) {
        const out = [];
        for (let page = 1; page <= maxPages; page += 1) {
            const rows = (await this.api(token, `/user/repos?affiliation=owner,collaborator,organization_member&sort=updated&per_page=${REPOS_PAGE_SIZE}&page=${page}`));
            if (!Array.isArray(rows))
                break;
            for (const row of rows)
                out.push(repositoryOf(row));
            if (rows.length < REPOS_PAGE_SIZE)
                break;
        }
        return out;
    }
     ...
     */
    async branches(token, owner, repo, maxPages = BRANCHES_MAX_PAGES) {
        const out = [];
        for (let page = 1; page <= maxPages; page += 1) {
            const rows = (await this.api(token, `/repos/${owner}/${repo}/branches?per_page=${REPOS_PAGE_SIZE}&page=${page}`));
            if (!Array.isArray(rows))
                break;
            for (const row of rows) {
                out.push({
    ...
    async pullRequestsFor(token, owner, repo, branch) {
        const rows = (await this.api(token, `/repos/${owner}/${repo}/pulls?state=open&head=${encodeURIComponent(`${owner}:${branch}`)}`));
        if (!Array.isArray(rows))
            return [];
        return rows.map((row) => ({
    ...
    async checks(token, owner, repo, ref) {
        const body = (await this.api(token, `/repos/${owner}/${repo}/commits/${encodeURIComponent(ref)}/check-runs?per_page=100`));
    ...
    async api(token, path, init = {}) {
        ...
        try {
            ...
            throw new GitHubError(`Sem conexão com o GitHub: ${error.message}`, 0, 'network');
        }
...
export function parseAnswerBody(raw, contentType) {
    const text = raw.trim();
    if (text.length === 0)
        return {};
...
function describeAnswer(answer) {
    ...
    if (typeof answer.body.error === 'string')
        parts.push(`error: ${answer.body.error}`);
    if (typeof answer.body.error_description === 'string')
        parts.push(`error_description: ${answer.body.error_description}`);
    if (answer.excerpt)
        parts.push(`body: ${answer.excerpt}`);
    return parts.join(' · ');
}
/**
 ...
 */
function classifyDeviceStart(answer) {
    const detail = describeAnswer(answer);
    ...
    if (answer.status === 404 || /^not found$/i.test(error) || error === 'unauthorized_client' || error === 'incorrect_client_credentials') {
        return new GitHubError('O GitHub não reconhece este Client ID. Confira em GitHub → Settings → Developer settings → GitHub Apps → seu app: ' +
            'o Client ID começa com "Iv1." ou "Iv23li" — não é o App ID (número) nem o Client secret.', answer.status || 404, 'config', detail);
    }
    if (error === 'device_flow_disabled') {
        return new GitHubError('O fluxo de dispositivo está desligado neste GitHub App. Na página do app no GitHub, marque "Enable Device Flow" e salve.', answer.status || 400, 'config', detail);
    }
    ...
    if (answer.contentType.includes('text/html')) {
        return new GitHubError(`O GitHub devolveu uma página em vez da resposta do login (HTTP ${answer.status}). Um proxy, firewall ou antivírus pode estar interferindo na conexão.`, answer.status, 'network', detail);
    }
    if (description || error) {
        return new GitHubError(`O GitHub não iniciou o login: ${description || error}.`, answer.status || 400, 'auth', detail);
    }
    return new GitHubError(`O GitHub não iniciou o login: a resposta não trouxe o código do dispositivo (HTTP ${answer.status}).`, answer.status || 400, 'api', detail);
}
...
export function parseGitHubRemote(remoteUrl) {
    if (!remoteUrl)
        return null;
    const match = /^(?:https:\/\/(?:[^@/]+@)?github\.com\/|git@github\.com:|ssh:\/\/git@github\.com\/)([^/\s]+)\/([^/\s]+?)(?:\.git)?\/?$/i.exec(remoteUrl.trim());
    if (!match)
        return null;
