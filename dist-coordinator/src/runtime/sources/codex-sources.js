...
import { fetchChecksums, fetchRelease, findAsset } from './github-releases.js';
...
export function codexTargetTriple(target) {
 ...
 */
export class CodexGitHubReleaseSource {
    fetchImpl;
    repository;
    env;
    id = 'codex-github-releases';
    label = 'Releases oficiais do openai/codex';
    contract = 'DOCUMENTED';
    /** The release publishes a SHA-256 manifest covering the package archives. */
    integrityStrategy = 'SHA256';
    constructor(fetchImpl = fetch, repository = CODEX_REPOSITORY, env = process.env) {
 ...
 * code change.
 */
export function codexAssetCandidates(triple, target) {
    const candidates = [
 ...
 */
export class CodexNpmRegistrySource {
    fetchImpl;
    id = 'codex-npm-registry';
    label = 'Pacote de plataforma no registry npm';
 ...
 */
export function defaultCodexSources(fetchImpl = fetch) {
    return [new CodexGitHubReleaseSource(fetchImpl), new CodexNpmRegistrySource(fetchImpl)];
}
//# sourceMappingURL=codex-sources.js.map