 ...
 */
export async function fetchRelease(repository, tag, fetchImpl = fetch, env = process.env) {
    ...
    if (!release)
        throw new Error(`GitHub API answered a release without a tag for ${url}`);
    return release;
}
 ...
 */
export async function fetchChecksums(url, fetchImpl = fetch) {
