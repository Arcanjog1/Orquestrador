...
import { closeSync, copyFileSync, createReadStream, mkdirSync, openSync, readFileSync, readSync, statSync, } from 'node:fs';
import { basename, dirname, join } from 'node:path';
import { buildSpawnPlan } from '../process/process-manager.js';
import { removeTreeWithRetry } from './fs-retry.js';
/** States that mean "the bytes are fine; this machine could not run them". */
const LOCAL_EXECUTION_STATES = new Set([
    'FILE_LOCKED',
    'ACCESS_DENIED',
    'SPAWN_FAILED',
 ...
 * lower-casing here.
 */
export const CPU_OVERRIDE_ENV_NAMES = ['OPENSSL_ia32cap', 'OPENSSL_armcap'];
const CAPABILITY_ABORT = /Fatal Error: HW capability found: (0x[0-9A-Fa-f]+) (0x[0-9A-Fa-f]+), but HW capability requested: (0x[0-9A-Fa-f]+) (0x[0-9A-Fa-f]+)/;
/** The exact AWS-LC signature, from stderr; null when it is not there. */
export function parseCapabilityAbort(stderr) {
 ...
 */
export function dropEnvKeys(env, names) {
...
export function cpuOverridesIn(env) {
 ...
 */
export function isLocalExecutionFailure(state) {
...
/* ----------------------------------------------------------------- static */
export async function inspectExecutable(path, options = {}) {
    ...
    if (options.hash !== false)
        result.sha256 = await sha256Of(path);
...
function detectFormat(head) {
    ...
    if (head.length >= 2 && head[0] === 0x23 && head[1] === 0x21)
        return 'script';
    if (/^@echo|^@rem|^rem |^echo /i.test(head.subarray(0, 8).toString('latin1')))
        return 'script';
    return 'unknown';
}
/** Enough of the PE header to know what the file is, without a PE library. */
export function readPe(head) {
    if (head.length < 0x40)
        return null;
    const peOffset = head.readUInt32LE(0x3c);
    if (peOffset + 24 > head.length)
        return null;
    if (head.readUInt32BE(peOffset) !== 0x50450000)
        return null; // "PE\0\0"
    const machineCode = head.readUInt16LE(peOffset + 4);
    const optionalSize = head.readUInt16LE(peOffset + 20);
    const optional = peOffset + 24;
    if (optionalSize < 2 || optional + optionalSize > head.length)
        return null;
...
function readZoneIdentifier(path) {
    try {
...
export function classifyRun(result, versionLine) {
    ...
    if (parseCapabilityAbort(result.stderr))
        return 'CPU_CAPABILITY_OVERRIDE_INCOMPATIBLE';
    ...
    if (result.outcome === 'timeout') {
        if (trace.stdoutBytes === 0 && trace.stderrBytes === 0)
            return 'PROCESS_STARTED_NO_OUTPUT';
        if (trace.stdoutBytes === 0)
            return 'PROCESS_STARTED_STDERR_ONLY';
        return 'PROCESS_HUNG';
    }
    if (result.signal !== null)
        return 'PROCESS_KILLED';
    if (result.exitCode === 0)
        return versionLine ? 'OK' : 'PROCESS_EXITED_NO_VERSION';
    if (result.exitCode !== null && (result.exitCode >>> 0) >= 0xc0000000)
        return 'PROCESS_CRASHED';
    return 'PROCESS_EXITED_NONZERO';
}
/** A version line: the first non-empty line of stdout, else of stderr. */
export function versionLineOf(stdout, stderr) {
    const line = (stdout || stderr).split(/\r?\n/).find((l) => l.trim().length > 0)?.trim();
    return line && line.length > 0 ? line : null;
}
/** Names and safe values: paths are fine, proxy URLs (credentials) are not. */
export function summariseEnv(env, extra = []) {
    const out = {};
    const byLower = new Map(Object.entries(env).map(([k, v]) => [k.toLowerCase(), v]));
...
function runFromResult(label, via, argv, cwd, env, result) {
    ...
    const firstAt = [trace.firstStdoutAt, trace.firstStderrAt].filter((t) => t !== null).map(Date.parse);
    const notes = [];
    if (trace.streamsLingered)
        notes.push('o processo saiu, mas os pipes ficaram abertos: outro processo herdou os handles');
    if (trace.survivedTermination)
        notes.push('o processo continuou vivo depois de todas as tentativas de encerrar');
    ...
    return {
        ...
        termination: trace.termination && trace.termination.attempts.length > 0
...
export async function rawSpawn(options) {
    ...
    try {
        ...
        return base({ outcome: 'spawn-error', error: error.message });
    }
    let child;
    try {
        ...
        trace.errorAt = new Date().toISOString();
        trace.errorCode = error.code ?? null;
        return base({ outcome: 'spawn-error', error: error.message });
    }
    ...
    return new Promise((resolve) => {
        ...
        const recordKill = () => {
            if (!killMethod || !trace.termination || trace.termination.attempts.length > 0)
                return;
 ...
 * comparisons showed.
 */
export async function probeExecution(options) {
    ...
    const viaManager = async (label, cwd, overlay, timeoutMs) => {
    ...
    if (first.state === 'OK') {
        if (first.durationMs >= slowStartMs) {
            ...
            if (second.state === 'OK' && second.durationMs * 4 < first.durationMs) {
                ...
                probe.conclusions.push(`a primeira execução levou ${seconds(first.durationMs)} e a segunda ${seconds(second.durationMs)}: o primeiro início foi retido (antivírus varrendo o arquivo novo)`);
            }
        }
        return probe;
    }
    ...
    if (first.capability) {
        ...
        if (without.state === 'OK') {
            ...
            probe.conclusions.push(`sem ${overrides.map((o) => o.key).join('/') || 'OPENSSL_ia32cap'} no ambiente do processo filho o executável respondeu (${without.versionLine}): a causa é a variável ${shown}, não o processador`);
            return probe;
        }
        probe.conclusions.push(`mesmo sem OPENSSL_ia32cap no processo filho o executável falhou (${without.state}); ambiente visto pelo aplicativo: ${shown}`);
    }
    ...
    if (second.state === 'OK') {
        ...
        probe.antivirusDelaySuspected = true;
        probe.conclusions.push(`a segunda execução respondeu em ${seconds(second.durationMs)} depois de a primeira ficar ${seconds(first.durationMs)} sem resposta: o primeiro início do arquivo novo foi retido (antivírus)`);
        return probe;
    }
    ...
    if (options.scratchRoot) {
        ...
        try {
            ...
            if (copied.sha256 !== probe.static.sha256)
                copyRun.notes.push(`a cópia tem outro SHA-256 (${copied.sha256})`);
            ...
            probe.conclusions.push(`a cópia em pasta controlada não pôde ser feita: ${error.message}`);
        }
        finally {
...
export function describeProbe(probe, platform = process.platform) {
    ...
    const facts = [];
    if (s.bytes !== null)
        facts.push(`${s.bytes.toLocaleString('pt-BR')} bytes`);
    if (s.sha256)
        facts.push(`sha256 ${s.sha256.slice(0, 8)}…${s.sha256.slice(-4)}`);
    if (s.pe)
        facts.push(`PE ${s.pe.machine} ${s.pe.subsystem}${s.pe.is64 ? '' : ' 32-bit'}`, `assinado: ${s.pe.signed ? 'sim' : 'não'}`);
    else if (s.format)
        facts.push(`formato: ${s.format}`);
    if (s.openForWrite !== null)
        facts.push(s.openForWrite ? 'sem outro handle aberto' : `abrir para escrita falhou (${s.openError})`);
    if (platform === 'win32')
        facts.push(`Zone.Identifier: ${s.zoneIdentifier ?? 'não'}`);
    lines.push(`executável: ${s.path}${facts.length ? ` (${facts.join(' · ')})` : ''}`);
    if (s.problem)
        lines.push(`problema: ${s.problem}`);
    probe.runs.forEach((run, index) => {
        ...
        if (run.firstOutputAfterMs !== null)
            parts.push(`primeiro byte após ${seconds(run.firstOutputAfterMs)}`);
        if (run.errorCode)
            parts.push(`erro ${run.errorCode}`);
        if (run.error && !run.errorCode)
            parts.push(`erro "${run.error}"`);
        if (run.termination)
            parts.push(`encerramento: ${run.termination}`);
    ...
    for (const c of probe.conclusions)
        lines.push(`conclusão: ${c}`);
...
function headline(probe, os) {
    ...
    switch (probe.state) {
        ...
        case 'CPU_CAPABILITY_OVERRIDE_INCOMPATIBLE': {
            ...
            return (`Esta versão do Codex não consegue iniciar neste computador com a variável de ambiente OPENSSL_ia32cap: ` +
...
export function probeFailedLocally(probe) {
    return !probe.recovered && probe.state !== 'OK' && isLocalExecutionFailure(probe.state);
}
//# sourceMappingURL=execution-probe.js.map