...
export const ALLOWED_ACTIONS = ['delegate', 'verify', 'done', 'blocked'];
/**
...
export function parseDecision(raw) {
    ...
    try {
        ...
        return fail(`The JSON object could not be parsed: ${err.message}`, raw);
    }
    ...
    if (!ALLOWED_ACTIONS.includes(action)) {
        return fail(`"${action}" is not an allowed action. Use one of: ${ALLOWED_ACTIONS.join(', ')}.`, raw);
    }
    const decision = {
        action: action,
        acceptanceCriteria: [],
    ...
    if (obj.summary !== undefined && obj.summary !== null) {
        if (typeof obj.summary !== 'string')
            return fail('"summary" must be a string.', raw);
    ...
    if (requirements)
        decision.workerRequirements = requirements;
    return { ok: true, decision };
}
function readWorkerRequirements(value) {
...
function readStringArray(value, field) {
    ...
    if (!Array.isArray(value))
        return { value: [], error: `"${field}" must be an array of strings.` };
    const out = [];
 ...
 */
export function buildRepairPrompt(error, raw) {
    return [
