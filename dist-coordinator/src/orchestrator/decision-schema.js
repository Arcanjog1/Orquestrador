...
export const DECISION_JSON_SCHEMA = {
    ...
    properties: {
        ...
        verificationCommands: {
            type: 'array',
            items: { type: 'string' },
            description: 'Verification ids registered for this workspace. Never a command line: an id that is ' +
        ...
        workerRequirements: {
            ...
            description: 'What the coding agent needs for THIS task, as tiers - never a model name. ' +
...
export function strictSchemaProblems(schema, path = '$') {
    ...
    if (types.includes('object')) {
        ...
        for (const key of keys)
            problems.push(...strictSchemaProblems(properties[key], `${path}.${key}`));
    }
    if (types.includes('array')) {
        if (node.items === undefined)
            problems.push(`${path}: arrays must declare items`);
        else
            problems.push(...strictSchemaProblems(node.items, `${path}[]`));
    }
    for (const keyword of Object.keys(node)) {
        if (!STRICT_KEYWORDS.has(keyword))
            problems.push(`${path}: "${keyword}" is not a supported keyword`);
    }
    return problems;
}
