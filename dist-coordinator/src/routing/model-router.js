 ...
 */
import { candidateSequence, resolveEffortForTier, resolveFixedEffort, } from './provider-policy.js';
import { assessTask, isSensitive } from './task-assessment.js';
import { DEFAULT_REQUIREMENTS, higherCapability, higherReasoning, promoteCapability, promoteReasoning, } from './tiers.js';
export function routeWorkerModel(input) {
    ...
    if (!input.requested) {
        ...
        reasons.push(`Codex pediu ${requested.capability}/${requested.reasoning}` +
    ...
    if (floored !== capability || flooredReasoning !== reasoning) {
        reasons.push(`promovido para ${floored}/${flooredReasoning} (${assessment.signals.join(', ')})`);
    }
    capability = floored;
    ...
    if (streak > 0) {
        ...
        if (escalated !== capability || escalatedReasoning !== reasoning) {
            reasons.push(`escalado para ${escalated}/${escalatedReasoning} após ${streak} tentativa(s) sem progresso`);
        }
    ...
    if (input.selection === 'quality') {
        ...
        if (up !== capability || upReasoning !== reasoning)
            reasons.push(`estratégia qualidade: ${up}/${upReasoning}`);
        capability = up;
        ...
        if (!isSensitive(assessment) && streak === 0) {
            ...
            if (down !== capability || downReasoning !== reasoning)
                reasons.push(`estratégia velocidade: ${down}/${downReasoning}`);
            capability = down;
 ...
 */
export function noProgressStreak(attempts) {
    let streak = 0;
    for (let index = attempts.length - 1; index >= 0; index -= 1) {
        const attempt = attempts[index];
        if (attempt.mechanical || attempt.modelUnavailable || attempt.outcome === 'cancelled')
            continue;
...
export function describeTiers(capability, reasoning) {
