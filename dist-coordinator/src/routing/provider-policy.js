...
import { CAPABILITY_TIERS, capabilityRank, } from './tiers.js';
...
const UNIVERSAL_EFFORTS = ['low', 'medium', 'high'];
 ...
 */
export function candidateSequence(provider, tier, unavailable = []) {
    const table = provider === 'anthropic' ? CLAUDE_MODELS : null;
    if (!table)
        return [];
    const own = capabilityRank(tier);
    const ranks = [own];
    for (let rank = own + 1; rank < CAPABILITY_TIERS.length; rank += 1)
        ranks.push(rank);
    for (let rank = own - 1; rank >= 0; rank -= 1)
        ranks.push(rank);
    const out = [];
    for (const rank of ranks) {
        const at = CAPABILITY_TIERS[rank];
        for (const model of table[at]) {
            if (unavailable.includes(model) || out.some((entry) => entry.model === model))
                continue;
 ...
 */
export function resolveEffortForTier(tier, declared) {
 ...
 */
export function resolveFixedEffort(saved, supported) {
    if (!saved)
        return { value: null, fallbackUsed: false, note: null };
    const accepted = supported ?? UNIVERSAL_EFFORTS;
    if (accepted.includes(saved))
        return { value: saved, fallbackUsed: false, note: null };
 ...
 * set is used).
 */
export function codexSupportedEfforts(version) {
