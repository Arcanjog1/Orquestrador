...
export class ContainerProcessRunner {
    host;
    runtimeCommand;
    container;
    inFlight = new Set();
    stopped = false;
    constructor(host, runtimeCommand, container) {
        this.host = host;
...
function abortedResult(options, error) {
...
export class ContainerWorkspaceProvisioner {
    options;
    id = 'container';
    capabilities = {
        isolated: true,
    ...
    async provision(request) {
        ...
        if (create.exitCode !== 0) {
            throw new ProvisioningError('BACKEND_UNAVAILABLE', 'Não foi possível criar o ambiente remoto.', firstLine(create.stderr) ?? `saída ${create.exitCode}`);
        }
        ...
        try {
            ...
            if (start.exitCode !== 0) {
                throw new ProvisioningError('BACKEND_UNAVAILABLE', 'O ambiente remoto não iniciou.', firstLine(start.stderr) ?? `saída ${start.exitCode}`);
            }
    ...
    async clone(runner, request) {
        ...
        if (request.privateRepository) {
            ...
            if (written.exitCode !== 0) {
                throw new ProvisioningError('CLONE_FAILED', 'Não foi possível preparar o acesso ao repositório dentro do ambiente.', firstLine(written.stderr));
            }
...
function cloneFailure(stderr, request) {
    const detail = firstLine(stderr);
    if (/could not read Username|Authentication failed|403|invalid credentials/i.test(stderr)) {
        return new ProvisioningError('REPOSITORY_UNAUTHORIZED', `O acesso ao repositório ${request.repository} não foi autorizado.`, detail);
    }
    ...
    if (/repository (?:'[^']*' )?not found|404/i.test(stderr)) {
        return new ProvisioningError('REPOSITORY_NOT_FOUND', `O repositório ${request.repository} não foi encontrado, ou esta instalação não tem acesso a ele.`, detail);
    }
    if (/Remote branch .* not found|couldn't find remote ref/i.test(stderr)) {
        return new ProvisioningError('BRANCH_NOT_FOUND', `A branch ${request.branch} não existe em ${request.repository}.`, detail);
    }
...
//# sourceMappingURL=container-provisioner.js.map