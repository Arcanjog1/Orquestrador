...
export class ClaudeAccountManager {
    ...
    async getStatus(account) {
        ...
        if (!this.hasOwnCredentials(account.id)) {
            return {
                ...
                ...(status.authMethod ? { authMethod: status.authMethod } : {}),
                problem: `${account.displayName} está usando uma credencial do sistema, não a própria. ` +
     ...
     */
    async verifyProfileInUse(account) {
        let executable;
        try {
            executable = await this.runtimeManager.getExecutablePath('claude-code');
        }
    ...
    async readAuthStatus(executable, accountId) {
...
//# sourceMappingURL=claude-account-manager.js.map