...
import { accountProfileDir, appPaths, ensureAppPaths } from '../runtime/paths.js';
import { AccountError, } from './account-types.js';
/**
...
export class CodexAccountManager {
    ...
    profileDirectory(accountId) {
        ...
        if (!isAbsolute(dir)) {
            throw new AccountError(accountId, 'Não foi possível preparar a pasta desta conta.', 'Tentar novamente', 'the resolved profile directory is not absolute');
        }
        return dir;
    }
    ...
    removeAccount(accountId) {
        ...
        if (!dir.startsWith(this.paths.profiles)) {
            throw new AccountError(accountId, 'Não foi possível remover esta conta.', 'Tentar novamente', 'refusing to delete a path outside the application profiles folder');
        }
    ...
    async getStatus(account) {
        ...
        if (!this.hasOwnCredentials(account.id)) {
            return {
                ...
                problem: 'Entrou com uma credencial que não é desta conta. Conecte novamente para isolá-la.',
    ...
    async connect(account, options = {}) {
        ...
        while (status.state !== 'connected' &&
            ...
            if (!reportedCode && extractDeviceCode(output))
                waiting();
    ...
    async supportsDeviceAuth(executable) {
        const result = await this.processManager.run({
            command: executable,
            args: ['login', '--help'],
            cwd: this.paths.root,
...
//# sourceMappingURL=codex-account-manager.js.map