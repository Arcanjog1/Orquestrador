...
export async function runPreflight(options) {
    ...
    const checks = [];
    checks.push(await checkTool(pm, {
        label: 'Node.js',
    ...
    if (options.checkCodex) {
        checks.push(await checkTool(pm, {
            ...
            cwd: options.cwd,
            timeoutMs,
            hint: 'Install the Codex CLI and authenticate it, or point `codexCommand` in config.json ' +
    ...
    if (options.checkClaude) {
        checks.push(await checkTool(pm, {
            ...
            cwd: options.cwd,
            timeoutMs,
            hint: 'Install the Claude Code CLI and authenticate it, or point `claudeCommand` in ' +
...
async function checkTool(pm, opts) {
 ...
 */
export async function resolveExecutable(command, pm = new ProcessManager()) {
...
export function scanPath(command, pathValue = process.env.PATH ?? process.env.Path, platform = process.platform, pathExt = process.env.PATHEXT) {
    if (!pathValue)
        return null;
