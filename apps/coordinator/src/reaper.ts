...
import type { Database } from '../../desktop/src/main/core.js';
import type { WorkspaceProvisioner } from '../../../src/cloud/provisioner.js';
import type { RunStore } from '../../../src/cloud/coordinator/store.js';

export interface ReaperOptions {
  database: Database;
  store: RunStore;
  provisioner: WorkspaceProvisioner;
  /** How often to look. Every minute is enough for hour-scale ceilings. */
  intervalMs?: number;
  /** Overridden in tests. */
  now?: () => Date;
}
...
export interface SweepResult {
  /** Workspaces past their ceiling, reclaimed. */
  expired: string[];
  /** Workspaces whose run has finished but which were still holding resources. */
  orphaned: string[];
}
...
export class Reaper {
  ...
  async sweep(): Promise<SweepResult> {
    ...
    const result: SweepResult = { expired: [], orphaned: [] };
    ...
    for (const workspace of this.options.database.cloudWorkspaces.listLive()) {
      const run = this.options.database.driver.get<{ status: string }>(
        'SELECT status FROM remote_runs WHERE cloud_workspace_id = ?',
        [workspace.id],
      );
      if (!run) continue;
      if (!['DONE', 'FAILED', 'CANCELLED', 'NEEDS_HUMAN'].includes(run.status)) continue;
  ...
  private async reclaim(id: string, handle: string | null, why: string): Promise<void> {
    // By handle, with no live object: the process that made it is gone, which
    // is the whole situation the reaper exists for.
    if (handle) await this.options.provisioner.reclaim(handle).catch(() => {});
