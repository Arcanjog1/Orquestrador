...
test('audit: create, edit, disable and remove agents through the real renderer',async()=>{
 const window=await openWindow();
 const account=await window.webContents.executeJavaScript("window.api.accounts.create({name:'Audit shared account',provider:'anthropic'})");
...
test('phase2: role provider independence and fixed model policy persist through the real renderer',async()=>{
 const window=await openWindow();
 const account=await window.webContents.executeJavaScript("window.api.accounts.create({name:'Phase2 OpenAI',provider:'openai'})");
 ...
 await reloadWindow(window);
 assert.equal((await window.webContents.executeJavaScript('window.api.agents.manage().then(list=>list.find(a=>a.id==='+JSON.stringify(agent.id)+'))')).policy.primaryModel,'gpt-5.3-codex-spark');
 ...
 await click(window,'agents-policies-tab');await click(window,'model-rule-add');
...
async function formValue(window,id,value) {
 await window.webContents.executeJavaScript('(()=>{const e=document.querySelector('+JSON.stringify('[data-testid="'+id+'"]')+');const proto=e.tagName==="SELECT"?HTMLSelectElement.prototype:HTMLInputElement.prototype;Object.getOwnPropertyDescriptor(proto,"value").set.call(e,'+JSON.stringify(value)+');e.dispatchEvent(new Event(e.tagName==="SELECT"?"change":"input",{bubbles:true}));})()');
}

test('cards: dynamic account models, reasoning, policy locks and no manual IDs',async()=>{
 const window=await openWindow();const original=services.agentModels.bind(services);
 const a=services.accounts.create('Claude 1 · Cards','anthropic'),b=services.accounts.create('Claude 2 · Cards','anthropic');
 ...
 await click(window,'agent-allow-opus');await formValue(window,'agent-account',b.id);
 ...
 await nativeClick(window,'agent-save');
 const agent=await waitFor(async()=>services.agents.manage().find(a=>a.name==='Claude Designer'),10000,'Designer saved');assert.equal(agent.accountId,b.id);assert.deepEqual(agent.policy.allowedModels,['sonnet','haiku']);assert.equal(agent.policy.reasoning,'medium');assert.deepEqual(agent.policy.fallbackModels,['haiku']);
