...
export async function agentCardsSmoke(services:AppServices,window:BrowserWindow):Promise<string> {
 ...
 services.agentModels=async(accountId,role)=>decorateAgentModels(['sonnet','haiku'].map(id=>({id,provider:'anthropic',source:'runtime',reasoning:['low','medium','high'],accountAllowed:true})),services.database.accounts.require(accountId),services.agents.policies(),role);
 try {
  ...
  await evalJS("location.hash='#/configuracoes?tab=agents';location.reload();true");await loaded;
  ...
  if(process.env.AI_ORCHESTRATOR_SMOKE_REOPEN!=='1') {
   ...
   await wait("!!document.querySelector('[data-testid=agent-allow-sonnet]')");
   await click('agent-allow-sonnet');await click('agent-allow-haiku');await click('agent-mode-CONTROLLED_AUTO');await value('agent-reasoning','medium');
   if(await evalJS("document.querySelector('[data-testid=agent-model]').tagName!=='SELECT'||document.querySelector('[data-testid=agent-advanced]').open"))throw new Error('Manual model input or advanced options exposed.');
  ...
  if(agent?.accountId!==accountId||agent.role!=='DESIGNER'||agent.policy?.modelMode!=='CONTROLLED_AUTO'||agent.policy.primaryModel!=='sonnet'||agent.policy.allowedModels.join(',')!=='sonnet,haiku'||agent.policy.reasoning!=='medium'||agent.policy.fallbackModels.join(',')!=='haiku')throw new Error('Designer settings did not survive the real process restart.');
  await wait(`!!document.querySelector(${JSON.stringify('[data-testid="agent-edit-'+id+'"]')})`);await click('agent-edit-'+id);
  await wait("!!document.querySelector('[data-testid=agent-allow-haiku]')");
