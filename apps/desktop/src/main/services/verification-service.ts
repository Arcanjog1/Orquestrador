...
import type { Database, VerificationDefinitionRecord } from '../core.js';
import { screenCommand } from '../core.js';
import type { VerificationView } from '../../shared/ipc-contract.js';
...
export class VerificationService {
  ...
  list(workspaceId: string): VerificationView[] {
  ...
  create(input: {
    ...
    return toView(
      this.database.verifications.create({
        id: input.id,
  ...
  update(input: {
    ...
    const changes: { label?: string; command?: string; enabled?: boolean } = {};
    if (input.label !== undefined) changes.label = requireLabel(input.label);
    if (input.command !== undefined) changes.command = this.screen(input.command);
  ...
  remove(input: { workspaceId: string; id: string }): boolean {
    this.database.workspaces.require(input.workspaceId);
   ...
   */
  private screen(command: string): string {
    const trimmed = command.trim();
    if (trimmed.length === 0) throw new VerificationError('Escreva o comando da verificação.');
...
function requireLabel(label: string): string {
  ...
  if (trimmed.length === 0) throw new VerificationError('Escreva um nome para a verificação.');
  return trimmed;
}

function toView(record: VerificationDefinitionRecord): VerificationView {
  return {
    id: record.id,
