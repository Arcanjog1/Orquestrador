...
import {availabilityOf,MODEL_AVAILABILITY_LABELS} from '../../shared/model-availability.js';
...
export function decorateAgentModels(rows:ModelCatalogEntry[],account:CatalogAccount,config:PolicyConfiguration,role?:string):ModelCatalogEntry[] {
 ...
 return rows.filter(row=>row.provider===account.provider_id).map(row=>{
  ...
  else if(row.accountAllowed===false)blockedReason=MODEL_AVAILABILITY_LABELS.UNAVAILABLE;
  ...
  return {...row,availability:availabilityOf(row.accountAllowed),displayName:modelDisplayName(row.provider,row.id,row.displayName),premium,blockedReason,reasoning,capability:tier};
 });
}
