import { HeroPortrait, GuildBanner } from './HeroPortrait';
import { HEROES, heroKey } from '@shared/hero-identity';
import {MODEL_AVAILABILITY_LABELS,availabilityOf} from '@shared/model-availability';
...
export function AgentsCard({accounts,onChanged}:{accounts:readonly AccountView[];onChanged:()=>void}) {
 ...
 const [probeTarget,setProbeTarget]=useState<ManagedAgentView|null>(null);
 const [verificationNotes,setVerificationNotes]=useState<Record<string,string>>({});
 const verify=async(a:ManagedAgentView,probe=false)=>{setVerifying(a.id);try{const result=probe?await api.agents.testModel({agentId:a.id,accountId:a.accountId,modelId:(a.policy?.primaryModel??a.model)!,authorised:true}):await api.agents.verifyModels({agentId:a.id});setVerificationNotes(notes=>({...notes,[keyOf(result.accountId,(a.policy?.primaryModel??a.model)!)]:result.detail}));const roles=[...new Set(agents.filter(agent=>agent.accountId===result.accountId).map(agent=>agent.role))];for(const role of roles){const rows=await api.agents.models({accountId:result.accountId,role});setCatalogs(current=>({...current,[keyOf(result.accountId,role)]:rows}));}}catch(e){setError(messageOf(e));}finally{setVerifying(null);}};
 ...
 const changeConnection=(provider:AgentInputView['provider'],accountId:string)=>{
  if(!draft||!policy)return;
  setModels([]);setCatalogNote('');setLoadingModels(!!accountId);
  setDraft({...draft,provider,accountId,model:null,reasoning:null,policy:{...policy,primaryModel:'',allowedModels:[],blockedModels:[],fallbackModels:[],reasoning:null}});
 };
 ...
 const statusOf=(a:ManagedAgentView)=>{
  ...
  return {label:'Ativo',kind:'active',tone:'bg-emerald-400/10 text-emerald-500',note:''};
 };
 ...
 return <section className="mt-6 space-y-6" data-testid="agents-manager">
  <GuildBanner title="Sua guilda de agentes" subtitle="Diferentes habilidades. Uma equipe para transformar ideias em resultados."/>
  ...
  {page==='policies'?<ModelPoliciesCard/>:<>
   ...
   <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 2xl:grid-cols-3" data-testid="agent-card-grid">{filtered.map(a=>{
    ...
    return <article key={a.id} data-testid={`agent-card-${a.id}`} className="hero-card flex min-w-0 flex-col rounded-2xl border border-border bg-card p-5 shadow-sm transition-colors hover:border-primary/30">
     <div className="flex items-start justify-between gap-3"><HeroPortrait role={a.role} variant="sprite" size={112} state={state.kind==='active'?'idle':state.kind==='error'?'blocked':'offline'}/><span className={'rounded-full px-2.5 py-1 text-xs font-medium '+state.tone}>● {state.label}</span></div>
     <p className="hero-archetype">{HEROES[heroKey(a.role)??'programmer'].title}</p><h3 className="mt-2 break-words text-base font-semibold">{a.name}</h3><span className="mt-1 w-fit rounded-md bg-secondary px-2 py-1 text-xs text-secondary-foreground">{ROLE_NAMES[a.role]??a.role}</span>
     ...
     {primary&&catalog?.find(m=>m.id===primary)&&<p data-testid={`agent-availability-${a.id}`} className={'mt-3 text-xs '+(catalog.find(m=>m.id===primary)?.accountAllowed===false?'text-destructive':'text-muted-foreground')} role="status">{MODEL_AVAILABILITY_LABELS[availabilityOf(catalog.find(m=>m.id===primary)!.accountAllowed)]}</p>}
     {state.note&&<p className="mt-3 text-xs text-amber-500" role="status">{state.note}</p>}
     <Button variant="ghost" size="sm" className="mt-3 h-auto whitespace-normal text-left" disabled={verifying!==null||!accounts.some(c=>c.id===a.accountId)} data-testid={`agent-verify-${a.id}`} onClick={()=>void verify(a)}><RefreshCw className={'mr-2 size-3.5 shrink-0 '+(verifying===a.id?'animate-spin':'')}/>{verifying===a.id?'Verificando…':catalog?.find(m=>m.id===primary)?.verification?'Verificar novamente':'Verificar modelos desta conta'}</Button>
     {verificationNotes[keyOf(a.accountId,primary??'')]&&<p role="status" className="mt-2 text-xs text-muted-foreground">{verificationNotes[keyOf(a.accountId,primary??'')]}</p>}
     {primary&&catalog?.find(m=>m.id===primary)?.verification&&<p className="mt-1 text-xs text-muted-foreground">{catalog.find(m=>m.id===primary)?.accountAllowed===true?'Verificado em':'Última tentativa em'} {new Date(catalog.find(m=>m.id===primary)!.verification!.verifiedAt).toLocaleString()} · {accounts.find(c=>c.id===a.accountId)?.name}</p>}
     {primary&&(verificationNotes[keyOf(a.accountId,primary)]||catalog?.find(m=>m.id===primary)?.verification)&&catalog?.find(m=>m.id===primary)?.accountAllowed===null&&<div className="mt-2"><Button variant="secondary" disabled={verifying!==null} data-testid={`agent-test-${a.id}`} onClick={()=>setProbeTarget(a)}>Testar {label(primary)}</Button><p className="mt-2 text-xs text-muted-foreground">Será feita uma chamada mínima usando esta conta e este modelo. Isso pode consumir uma pequena quantidade do seu limite de uso.</p></div>}
   ...
   {!agents.length&&<div className="rounded-2xl border border-dashed p-10 text-center text-muted-foreground"><Bot className="mx-auto mb-3 size-8"/><p>Nenhum herói recrutado. Crie seu primeiro agente para montar a equipe.</p></div>}
  </>}
  <Dialog open={!!probeTarget} onOpenChange={open=>{if(!open)setProbeTarget(null);}}><DialogContent><DialogHeader><DialogTitle>Testar {probeTarget?modelDisplayName(probeTarget.provider,(probeTarget.policy?.primaryModel??probeTarget.model)!):''} nesta conta?</DialogTitle><DialogDescription>Uma chamada mínima será realizada e poderá consumir uma pequena quantidade do limite desta conta.</DialogDescription></DialogHeader>{probeTarget&&<><p>Conta: {accounts.find(c=>c.id===probeTarget.accountId)?.name}</p><p>Provider: {providerName(probeTarget.provider)}</p><p>Modelo: {modelDisplayName(probeTarget.provider,(probeTarget.policy?.primaryModel??probeTarget.model)!)}</p><div className="flex justify-end gap-2"><Button variant="ghost" data-testid="model-probe-cancel" onClick={()=>setProbeTarget(null)}>Cancelar</Button><Button data-testid="model-probe-confirm" disabled={verifying!==null} onClick={()=>{const target=probeTarget;setProbeTarget(null);void verify(target,true);}}>Testar</Button></div></>}</DialogContent></Dialog>
  <Dialog open={!!draft} onOpenChange={open=>{if(!open&&!busy)setDraft(null);}}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>{editing?'Editar agente':'Criar agente'}</DialogTitle><DialogDescription>Escolha a função, a conta e o modelo do agente.</DialogDescription></DialogHeader>
   {draft&&policy&&<form className="space-y-6" onSubmit={e=>{e.preventDefault();void save();}}>
    ...
    <fieldset className="grid grid-cols-1 gap-3 sm:grid-cols-2"><legend className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Conexão</legend><label className="text-sm">Provedor<select data-testid="agent-provider" className={selectClass} value={draft.provider} onChange={e=>changeConnection(e.target.value as AgentInputView['provider'],'')}><option value="openai">OpenAI</option><option value="anthropic">Anthropic</option></select></label><label className="text-sm">Conta<select required data-testid="agent-account" className={selectClass} value={draft.accountId} onChange={e=>changeConnection(draft.provider,e.target.value)}><option value="">Escolha uma conta</option>{accounts.filter(a=>a.provider===draft.provider).map(a=><option key={a.id} value={a.id}>{a.name}{a.state==='connected'?'':' · Desconectada'}</option>)}</select></label></fieldset>
    <fieldset className="space-y-4"><legend className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Modelo</legend>
     <label className="block text-sm">Modelo<select data-testid="agent-model" className={selectClass} required disabled={!draft.accountId||loadingModels} value={policy.primaryModel} onChange={e=>{const id=e.target.value,allowed=policy.modelMode==='FIXED'?[id]:[...new Set([id,...policy.allowedModels])];setDraft({...draft,model:id,reasoning:null,policy:{...policy,primaryModel:id,allowedModels:allowed,reasoning:null,fallbackModels:policy.modelMode==='CONTROLLED_AUTO'?allowed.filter(m=>m!==id):[],blockedModels:policy.blockedModels.filter(m=>m!==id)}});}}><option value="">{loadingModels?'Carregando modelos…':'Escolha um modelo'}</option>{models.map(m=><option key={m.id} value={m.id} disabled={!!m.blockedReason}>{m.displayName??modelDisplayName(m.provider,m.id)}{m.blockedReason?' — '+m.blockedReason:''}</option>)}{policy.primaryModel&&!models.some(m=>m.id===policy.primaryModel)&&<option value={policy.primaryModel} disabled>{modelDisplayName(draft.provider,policy.primaryModel)} — Indisponível</option>}</select></label>
     <div className="flex items-center justify-between gap-2"><span className="text-xs text-muted-foreground">{policy.modelMode==='CONTROLLED_AUTO'?'Modelos permitidos':'Modelos da conta selecionada'}</span><Button type="button" variant="ghost" size="sm" disabled={!draft.accountId||loadingModels} data-testid="agent-model-refresh" onClick={()=>setRevision(r=>r+1)}><RefreshCw className={'mr-1 size-3.5 '+(loadingModels?'animate-spin':'')}/>Atualizar modelos</Button></div>
     {!draft.accountId?<p className="text-sm text-muted-foreground">Selecione uma conta para ver seus modelos.</p>:loadingModels?<p role="status" className="text-sm text-muted-foreground">Buscando modelos desta conta…</p>:policy.modelMode==='CONTROLLED_AUTO'?<div className="grid grid-cols-1 gap-2 sm:grid-cols-2" data-testid="agent-model-options">{[...models,...policy.allowedModels.filter(id=>!models.some(m=>m.id===id)).map(id=>({id,provider:draft.provider,source:'catalog' as const,reasoning:[],accountAllowed:false,displayName:modelDisplayName(draft.provider,id),blockedReason:'Modelo indisponível nesta conta.'}))].map(m=><label key={m.id} className={'flex cursor-pointer items-start gap-2 rounded-xl border p-3 text-sm '+(policy.allowedModels.includes(m.id)?'border-primary/60 bg-primary/5':'border-border')+(m.blockedReason?' opacity-70':'')} title={m.blockedReason??undefined}><input type="checkbox" className="mt-1 accent-primary" data-testid={`agent-allow-${m.id}`} checked={policy.allowedModels.includes(m.id)} disabled={!!m.blockedReason&&!policy.allowedModels.includes(m.id)} onChange={()=>toggleModel(m.id)}/><span className="min-w-0"><span className="flex items-center gap-1 font-medium">{m.blockedReason&&<Lock className="size-3"/>}{m.displayName??modelDisplayName(m.provider,m.id)}{'premium' in m&&m.premium&&<span className="text-[10px] text-amber-500">Premium</span>}</span>{m.blockedReason&&<span className="mt-1 block text-xs">{m.blockedReason}</span>}</span></label>)}</div>:null}
     {models.find(m=>m.id===policy.primaryModel)&&<p className="text-xs text-muted-foreground" role="status">{MODEL_AVAILABILITY_LABELS[availabilityOf(models.find(m=>m.id===policy.primaryModel)!.accountAllowed)]}</p>}
     {catalogNote&&<p role="status" className="text-xs text-muted-foreground">{catalogNote}</p>}

     <div className="grid grid-cols-1 gap-2 sm:grid-cols-2" role="radiogroup" aria-label="Modo de modelo">{(['FIXED','CONTROLLED_AUTO'] as const).map(mode=><label key={mode} className={'cursor-pointer rounded-xl border p-3 text-sm '+(policy.modelMode===mode?'border-primary bg-primary/5':'border-border')}><input type="radio" className="mr-2 accent-primary" name="model-mode" data-testid={`agent-mode-${mode}`} checked={policy.modelMode===mode} onChange={()=>setDraft({...draft,policy:{...policy,modelMode:mode,fallbackModels:mode==='CONTROLLED_AUTO'?policy.allowedModels.filter(m=>m!==policy.primaryModel):[],reasoning:null},reasoning:null})}/>{mode==='FIXED'?'Modelo fixo':'Permitir modelos alternativos'}</label>)}</div>
     ...
     {policy.modelMode==='CONTROLLED_AUTO'&&<label className="block text-sm">Modelo alternativo<select className={selectClass} data-testid="agent-fallback" value={policy.fallbackModels.length>1?'all':policy.fallbackModels[0]??''} onChange={e=>setDraft({...draft,reasoning:null,policy:{...policy,reasoning:null,fallbackModels:e.target.value==='all'?policy.allowedModels.filter(m=>m!==policy.primaryModel):e.target.value?[e.target.value]:[]}})}><option value="">Sem modelo alternativo</option>{policy.allowedModels.filter(id=>id!==policy.primaryModel).length>1&&<option value="all">Todos os alternativos selecionados, na ordem</option>}{policy.allowedModels.filter(id=>id!==policy.primaryModel).map(id=><option key={id} value={id}>{models.find(m=>m.id===id)?.displayName??modelDisplayName(draft.provider,id)}</option>)}</select></label>}
    ...
    <div className="flex flex-wrap justify-end gap-2 border-t pt-4">{editing&&<Button type="button" variant="ghost" className="mr-auto text-destructive" data-testid={`agent-remove-${editing}`} disabled={busy} onClick={()=>void mutate(()=>api.agents.remove({agentId:editing})).then(ok=>{if(ok)setDraft(null);})}>Remover agente</Button>}<Button type="button" variant="ghost" disabled={busy} onClick={()=>setDraft(null)}>Cancelar</Button><Button type="submit" data-testid="agent-save" disabled={busy||!canSave}>{busy?'Salvando…':editing?'Salvar alterações':'Criar agente'}</Button></div>
   </form>}
