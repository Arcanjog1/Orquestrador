  ...
  {page==='policies'?<ModelPoliciesCard/>:<>
   ...
   <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 2xl:grid-cols-3" data-testid="agent-card-grid">{filtered.map(a=>{
    ...
    return <article key={a.id} data-testid={`agent-card-${a.id}`} className="flex min-w-0 flex-col rounded-2xl border border-border bg-card p-5 shadow-sm transition-colors hover:border-primary/30">
     ...
     <Button variant="ghost" size="sm" className="mt-3 h-auto whitespace-normal text-left" disabled={verifying!==null||!accounts.some(c=>c.id===a.accountId)} data-testid={`agent-verify-${a.id}`} onClick={()=>void verify(a)}><RefreshCw className={'mr-2 size-3.5 shrink-0 '+(verifying===a.id?'animate-spin':'')}/>{verifying===a.id?'Verificando…':'Verificar modelos desta conta'}</Button>
     {verificationNotes[a.accountId]&&<p role="status" className="mt-2 text-xs text-muted-foreground">{verificationNotes[a.accountId]}</p>}
