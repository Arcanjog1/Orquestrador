...
export function ExecutionWorktree({
  ...
  const [offset, setOffset] = useState({ x: 64, y: 40 });
  ...
  function focus(node: ExecutionNode) {
    setOffset({
      x: viewport.width / 2 - (node.lane * GAP + WIDTH / 2) * zoom,
      y: viewport.height / 2 - (node.row * ROW + HEIGHT / 2) * zoom,
    });
    select(node.id);
  }
  function fit() {
    const width = Math.max(...layout.map((n) => n.lane * GAP + WIDTH), WIDTH);
    const height = Math.max(...layout.map((n) => n.row * ROW + HEIGHT), HEIGHT);
    const next = Math.min(
      1.2,
      Math.max(
        0.15,
        Math.min(
          ...
          (viewport.height - 60) / height,
        ),
      ),
    );
    setZoom(next);
    setOffset({ x: (viewport.width - width * next) / 2, y: 30 });
  }
  ...
  const visible = layout.filter((n) => {
    const x = n.lane * GAP * zoom + offset.x,
      y = n.row * ROW * zoom + offset.y;
    return (
      x > -WIDTH * zoom - 200 &&
  ...
  const related = (direction: "parent" | "child") =>
    graph.edges
      ...
      .filter(Boolean);
  return (
    <div className="execution-worktree" data-testid="execution-worktree">
      <div className="worktree-summary">
        <span>
          <GitBranch size={14} /> {detail.run.status}
        </span>
        <span>
      ...
      <div className="worktree-body">
        <div
          ...
          tabIndex={0}
          aria-label="Grafo da execução. Arraste para navegar; use mais e menos para zoom."
        ...
        >
          ...
          >
            <svg
              ...
              width={Math.max(
                viewport.width / zoom,
                ...layout.map((n) => n.lane * GAP + WIDTH),
              )}
              height={Math.max(
                viewport.height / zoom,
                ...layout.map((n) => n.row * ROW + HEIGHT),
              )}
              aria-hidden="true"
            >
              {displayedEdges.map((edge) => {
                ...
                const x1 = from.lane * GAP + WIDTH / 2,
                  y1 = from.row * ROW + HEIGHT,
                  x2 = to.lane * GAP + WIDTH / 2,
                  y2 = to.row * ROW;
                return (
                  <path
                    key={edge.id}
              ...
              >
                ...
                >
                  ...
                  <span className="node-summary">
                    {node.summary || (node.finishedAt ? "Sem resposta registrada nesta etapa." : "Aguardando resultado desta etapa.")}
                  </span>
          ...
          <div className="worktree-legend">
            ● Codex <span>● Worker</span>
            <span>● Evidência</span> · Arraste para navegar
          </div>
        </div>
        {selectedNode && (
