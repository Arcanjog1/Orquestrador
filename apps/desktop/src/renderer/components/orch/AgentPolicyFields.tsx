...
export function AgentPolicyFields({value,role,onChange,reasoningOptions=[]}:{value:AgentPolicy;role:string;onChange:(value:AgentPolicy)=>void;reasoningOptions?:string[]}) {
 ...
 return <fieldset className="space-y-3 border-t pt-3" data-testid="agent-policy-fields"><legend className="font-medium">Política de execução</legend>
  <label className="block text-sm">Seleção do modelo<select className="block w-full rounded border bg-surface p-2" data-testid="agent-model-mode" value={value.modelMode} onChange={e=>update({modelMode:e.target.value as AgentPolicy['modelMode'],fallbackModels:[]})}><option>FIXED</option><option>CONTROLLED_AUTO</option></select></label>
  <p className="text-xs text-muted-foreground">{value.modelMode==='FIXED'?'Usa exatamente o modelo principal. Indisponibilidade bloqueia a chamada.':'Usa o principal e, se indisponível ou bloqueado, apenas os fallbacks abaixo, em ordem.'}</p>
  {(['allowedModels','blockedModels','fallbackModels'] as const).map(key=><label className="block text-sm" key={key}>{key==='allowedModels'?'Modelos permitidos':key==='blockedModels'?'Modelos bloqueados':'Fallbacks em ordem'}<ModelList testId={`agent-${key}`} disabled={key==='fallbackModels'&&value.modelMode==='FIXED'} value={value[key]} onChange={models=>update({[key]:models})}/></label>)}
  <div className="grid grid-cols-2 gap-3">{(['reasoning','reasoningCeiling'] as const).map(key=><label className="text-sm" key={key}>{key==='reasoning'?'Raciocínio solicitado':'Teto de raciocínio'}<select className="block w-full rounded border bg-surface p-2" value={value[key]??''} onChange={e=>update({[key]:e.target.value||null})}><option value="">Automático</option>{(key==='reasoning'?[...new Set([...reasoningOptions,...(value.reasoning?[value.reasoning]:[])])]:EFFORT_ORDER).map(level=><option key={level}>{level}</option>)}</select></label>)}</div>
...
export function PolicyLayerFields({value,onChange}:{value:PolicyLayer;onChange:(value:PolicyLayer)=>void}) {
 return <div className="grid grid-cols-2 gap-3">{(['allowedModels','blockedModels'] as const).map(key=><label className="text-xs" key={key}>{key==='allowedModels'?'Restringir aos modelos':'Bloquear modelos'}<Input value={value[key]?.join(', ')??''} placeholder="Sem restrição adicional" onChange={e=>{const next={...value};if(e.target.value.trim())next[key]=split(e.target.value);else delete next[key];onChange(next);}}/></label>)}
 ...
 {(['maxAttempts','timeoutMs','maxTokens','maxCostUsd'] as const).map(key=><label key={key} className="text-xs">{key}<Input type="number" min="1" value={value[key]??''} onChange={e=>{const next={...value};if(e.target.value)next[key]=Number(e.target.value);else delete next[key];onChange(next);}}/></label>)}
 </div>;
}
